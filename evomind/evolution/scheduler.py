...
class EvolutionScheduler:
    ...
    executor_history: List[Dict[str, object]] = field(default_factory=list)
    ...
    def run(self) -> List[Dict[str, float]]:
        ...
        with self.logger.start_run(self.config.run_name):
            for generation in range(self.config.generations):
                metrics, best_genome, lineage, exec_stats = self.engine.run_generation(
                ...
                if exec_stats:
                    self.executor_history.append(exec_stats)
        return self.history
